function net = initializeNetworkTwoStreams2(imdb, encoderOpts, opts)
...
concat_layer = {encoder.neta.layers{end}.name, strcat('netb_', encoder.netb.layers{end}.name)};
inputLayerIndex = net.getLayerIndex(concat_layer);
in1 = net.layers(inputLayerIndex(1)).outputs;
in2 = net.layers(inputLayerIndex(2)).outputs;
input = cat(2, in1, in2);
layerName = 'concat_1';
output = 'c_1';
net.addLayer(layerName, dagnn.Concat(), ...
    input, output);

%avg_pool layer
layerName = sprintf('gap');
input = output;
apoutput = 'ap';
net.addLayer(layerName, dagnn.GlobalPooling(), {input}, apoutput);
...
% input = bpoutput;
% dropout = 'dropout';
% net.addLayer(layerName, dagnn.DropOut('rate', 0.5), {input}, dropout);
...
net.addLayer(layerName, dagnn.Conv(), {apoutput}, 'score', {param(1).name param(2).name});
